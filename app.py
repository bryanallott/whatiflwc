...
leaderboards = load_baseline_leaderboards()

division = st.selectbox("Division", ["Youth Men", "Youth Women", "Open Men", "Open Women"])

# NOTE: athletes.json currently lacks country_code. For v1, we look it up from baseline_results
# (an athlete with the same name in the same division usually has competed somewhere).
# If not found, candidate is rejected with a warning. Alternative: add country_code to athletes.json.
def lookup_country(athlete_name, division):
    for r in baseline:
...
def real_pools(division, event_id):
    ...
    for r in baseline:
        ...
        e = Entry(athlete=r["athlete"], country_code=r["country_code"], time_sec=t,
                  prev_round=r["round"],
...
for event_id, event_name in div_events_sim:
    ...
    for a in active.values():
        ...
        cc = lookup_country(a["athlete"], division) or "RSA"
        cands.append(Entry(athlete=a["athlete"], country_code=cc,
...
for row in leaderboards[division]:
    ...
    lb_rows.append({"Country": row["country"], "Code": cc, "Baseline": base, "Simulated": sim, "Δ": sim - base})
